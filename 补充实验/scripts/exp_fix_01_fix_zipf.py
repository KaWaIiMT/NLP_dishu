...
def load_corpus_for_comparison(corpus_path, corpus_name, lang, target_tokens=50000):
    ...
    try:
        ...
        if len(words) > target_tokens:
            words = random.sample(words, target_tokens)
...
def run_analysis(output_path):
    print("=" * 60)
    print("EXP-FIX-01-FIX: Zipf's Law Analysis (Corrected Tokenization)")
    print("=" * 60)
    ...
    for corpus_dir, corpus_name, lang in corpora:
        ...
        result = load_corpus_for_comparison(corpus_path, corpus_name, lang)
        if result:
    ...
    output_data = {
        'analysis_type': 'Zipf Law Analysis (EXP-FIX-01-FIX) - Corrected Tokenization',
...
if __name__ == "__main__":
    output_path = str(BASE_DIR / "补充实验" / "results" / "zipf_analysis_fixed.json")
